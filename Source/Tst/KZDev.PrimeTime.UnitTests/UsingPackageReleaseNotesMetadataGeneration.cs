...
public class UsingPackageReleaseNotesMetadataGeneration
{
    ...
    private static readonly IReadOnlyDictionary<string, string> ExpectedPackageReleaseNotesByPackageId =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["KZDev.PrimeTime"] = "- KZDev.PrimeTime package baseline release summary for version `0.0.5`.",
            ["KZDev.SystemClock.PrimeTime"] = "- KZDev.SystemClock.PrimeTime package baseline release summary for version `0.0.5`.",
            ["KZDev.PrimeTime.Testing"] = "- KZDev.PrimeTime.Testing package baseline release summary for version `0.0.5`.",
            ["KZDev.SystemClock.PrimeTime.Testing"] = "- KZDev.SystemClock.PrimeTime.Testing package baseline release summary for version `0.0.5`."
        };
    ...
    /// <summary>
    ///   Verifies each package gets the expected package-specific release summary from the Package section.
    /// </summary>
    [Fact]
    public void PackageMetadataGeneration_ForEachPackage_MapsToExpectedPackageSection ()
    {
        foreach (KeyValuePair<string, string> expectedPackageReleaseNotes in ExpectedPackageReleaseNotesByPackageId)
        {
            string projectRelativePath = PackageProjectRelativePaths[expectedPackageReleaseNotes.Key];
            string fullProjectPath = Path.Combine(RepositoryRootPath, projectRelativePath);
            string packageReleaseNotes = GetPackageReleaseNotesFromMsbuild(fullProjectPath);
            packageReleaseNotes.Should().Be(expectedPackageReleaseNotes.Value,
                $"expected package '{expectedPackageReleaseNotes.Key}' to map to its own ### Package summary");
        }
    }
}
//################################################################################
