...
**KZDev.PrimeTime** is the full **PrimeTime** stack for .NET: a testable clock abstraction (`IPrimeClock`), production and test implementations, interval and (on modern .NET) local **time-of-day** timers with explicit daylight-saving behavior, and **NodaTime**-first APIs (`Instant`, `Duration`, `LocalTime`, `ZonedDateTime`, and timer overloads).
